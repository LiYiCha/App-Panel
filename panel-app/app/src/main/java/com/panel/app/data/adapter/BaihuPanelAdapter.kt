    ...
    private fun BaihuTaskItem.toUnifiedSubscription(): UnifiedSubscription {
        ...
        config?.let { raw ->
            runCatching {
                ...
                obj?.let {
                    repoUrl = it.get("repo_url")?.takeIf { !it.isJsonNull }?.asString ?: it.get("repourl")?.takeIf { !it.isJsonNull }?.asString ?: ""
                    branch = it.get("branch")?.takeIf { !it.isJsonNull }?.asString ?: "main"
                    whitelist = it.get("whitelist_paths")?.takeIf { !it.isJsonNull }?.asString ?: ""
